...
function normalizeResponse(parsed: any): NormalizedAgentResponse {
  ...
  // Mode-based response: {mode, response, data} — preserve full structure
  if ('mode' in parsed && typeof parsed.mode === 'string' && 'response' in parsed && 'data' in parsed) {
    return {
      status: 'success',
      result: parsed,
      message: typeof parsed.response === 'string' ? parsed.response : undefined,
    }
  }
...
async function submitTask(body: any) {
  ...
  const finalUserId = 'ddowling@prophet.com'
...
async function pollTask(task_id: string) {
  ...
  // Task failed — return 200 with error payload so the client can handle gracefully
  // (HTTP 500 triggers iframe error reporting which is misleading for agent-level failures)
  ...
  const rawText = typeof task.response === 'string' ? task.response : JSON.stringify(task.response)
  let moduleOutputs: ModuleOutputs | undefined
  let agentResponseRaw: any = task.response

  try {
    const envelope = typeof task.response === 'string' ? JSON.parse(task.response) : task.response

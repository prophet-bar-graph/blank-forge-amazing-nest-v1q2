...
function parseStructured(response: any): Variation[] {
  ...
  if (!Array.isArray(variations) && typeof response?.response === 'string') {
    try {
      ...
      if (Array.isArray(inner?.data?.variations)) variations = inner.data.variations
    } catch {}
  }
  if (!Array.isArray(variations)) return []
  return variations.map((v: any, i: number): Variation => {
    const scores = v?.scores || v?.score
    const parsedScores: LensScores | undefined = scores && typeof scores === 'object'
      ? {
          voice: Number(scores.voice ?? scores.voice_score ?? 0) || 0,
          messaging: Number(scores.messaging ?? scores.messaging_score ?? 0) || 0,
          strategy: Number(scores.strategy ?? scores.strategy_score ?? 0) || 0,
        }
      : (typeof v?.voice_score === 'number' || typeof v?.messaging_score === 'number')
        ? {
...
function parseMarkdownFallback(text: string): Variation[] {
  ...
  type Hit = { headerStart: number; bodyStart: number; n: string; diff: string }
  const hits: Hit[] = []
  ...
  while ((m = optionRe.exec(trimmed)) !== null) {
    ...
    hits.push({ headerStart, bodyStart, n: m[1], diff: m[2].trim().replace(/\*+/g, '').trim() })
  }
  if (hits.length < 2) return []

  return hits.map((h, i) => {
    const end = i + 1 < hits.length ? hits[i + 1].headerStart : trimmed.length
    const body = trimmed.slice(h.bodyStart, end)
      .trim()
      .replace(/^---+$/gm, '')
...
function deepText(value: any, depth = 0): string {
  ...
  if (typeof value === 'string') {
    ...
    if (trimmed.startsWith('{') || trimmed.startsWith('[')) {
      try { return deepText(JSON.parse(trimmed), depth + 1) } catch {}
    }
...
function parseWriteResponse(response: any): WriteResult {
  ...
  if (structured.length > 0) return { variations: structured, raw: deepText(response) }
...
function heuristicScores(copy: string, mandatories: string[], i: number): LensScores {
  const wordCount = copy.split(/\s+/).filter(Boolean).length
  const voiceBase = wordCount > 30 && wordCount < 250 ? 78 : 64
  const messagingBase = mandatories.length > 0 && mandatories.some(p => p && copy.toLowerCase().includes(p.toLowerCase())) ? 82 : 70
  const strategyBase = 75
  // Variation-specific seed so they don't all read identically
  const seeds: ReadonlyArray<readonly [number, number, number]> = [[0, 4, -3], [-2, -1, 2], [3, -3, 1]]
...
// Strip markdown emphasis (**bold**, *italic*, ***bold-italic***). Belt-and-suspenders so
// asterisks never leak into the rendered card even if the agent's output format varies.
function stripMarkdownEmphasis(text: string): string {
  return text
    .replace(/\*{3}([^*\n]+?)\*{3}/g, '$1')
    .replace(/\*{2}([^*\n]+?)\*{2}/g, '$1')
    .replace(/(?<!\*)\*([^*\n]+?)\*(?!\*)/g, '$1')
    // Strip orphan ** markers left at text boundaries by the headline/body split or by
    // unbalanced agent output. Only matches when the ** is bounded by whitespace, newlines,
    // or the start/end of the string — i.e., not adjacent to real content characters.
    .replace(/(^|\s)\*{2}(?=\s|$)/g, '$1')
    .trim()
}
...
function parseEmailFormat(copy: string): { subject: string | null; body: string } {
  ...
  let body = copy.slice(m[0].length).replace(/^\s*\*{0,2}\s*Body\s*\*{0,2}\s*:\s*\n*/i, '')
  return { subject, body: body.trim() }
}

function HighlightedCopy({ copy, mandatories }: { copy: string; mandatories: string[] }) {
  ...
  const escaped = matches.sort((a, b) => b.length - a.length).map(s => s.replace(/[.*+?^${}()|[\]\\]/g, '\\$&'))
  ...
  return (
    <>
      {parts.map((p, i) => {
        const isMatch = i % 2 === 1
        return isMatch
          ? <mark key={i} className="bg-studio-scoreGold/20 text-studio-ink underline decoration-studio-scoreGold/70 underline-offset-[3px] decoration-2 px-0.5 rounded-sm">{p}</mark>
...
export default function WriteSection({ channel, audience, onCallAgent, loading, onSendToRefine, onChannelChange, onAudienceChange }: WriteSectionProps) {
  ...
  const handleGenerate = async () => {
    ...
    const prompt = [
      ...
      mandatories.length ? `Mandatory phrases (must appear verbatim, at least once across the variations): ${mandatories.join(' | ')}` : '',
      '',
    ...
    if (response) {
      setResult(parseWriteResponse(response))
      setCarouselIndex(0)
    } else {
  ...
  const enrichedVariations = useMemo(() => {
    if (!result) return []
    return result.variations.map((v, i) => {
      const scores = v.scores || heuristicScores(v.copy, mandatories, i)
      const wordCount = v.word_count ?? v.copy.split(/\s+/).filter(Boolean).length
      const fit = scores.voice + scores.messaging + scores.strategy
      return { ...v, scores, word_count: wordCount, fit }
    }).sort((a, b) => b.fit - a.fit)
  }, [result, mandatories])

  const handleRegenerateOne = async (differentiator: string, previousCopy: string) => {
    if (!contentObjective.trim() || !supportingMessages.trim() || !result) return
    ...
    const prompt = [
      ...
      supportingMessages ? `Supporting Messages: ${supportingMessages}` : '',
      callToAction ? `Call to Action: ${callToAction}` : '',
      mandatories.length ? `Mandatory phrases (must appear verbatim): ${mandatories.join(' | ')}` : '',
      '',
      ...
      if (fresh) {
        ...
        setResult(prev => prev ? ({
          ...
          variations: prev.variations.map(v =>
            v.differentiator.toLowerCase() === differentiator.toLowerCase() ? updatedFresh : v
          ),
        }) : prev)
      } else {
  ...
  const safeIndex = Math.min(carouselIndex, Math.max(0, enrichedVariations.length - 1))
    ...
    <div className="grid grid-cols-1 lg:grid-cols-[minmax(0,1fr)_minmax(0,2fr)] gap-6 lg:gap-8">
      ...
      <div className="flex flex-col">
        ...
        <div className="space-y-2.5 flex-1">
          ...
          {/* 3. Content type */}
          <div>
            <h4 className="font-bold text-sm text-studio-ink">Content Type:</h4>
            <p className="text-xs italic text-studio-mutedSoft mb-1">What do we want to write?</p>
            <Input
          ...
          <div>
            <h4 className="font-bold text-sm text-studio-ink">Supporting Messages:</h4>
            <p className="text-xs italic text-studio-mutedSoft mb-1">What are the key themes and messages to include?</p>
            <Input
          ...
          {/* 5. Call to action */}
          <div>
            <h4 className="font-bold text-sm text-studio-ink">Call to action:</h4>
            <p className="text-xs italic text-studio-mutedSoft mb-1">What do we want our audience to do?</p>
            <Input
              value={callToAction}
          ...
          <div>
            <div className="flex items-baseline gap-1.5">
              <h4 className="font-bold text-sm text-studio-ink">Mandatories</h4>
              <span className="text-xs italic text-studio-mutedSoft">· optional</span>
            </div>
            <p className="text-xs italic text-studio-mutedSoft mb-1">Key phrase or statistic</p>
            <Input
              value={mandatories.join('\n')}
              onChange={(e) => setMandatories(e.target.value ? [e.target.value] : [])}
          ...
          <div>
            ...
            <p className="text-sm font-bold text-studio-ink mt-1">{tone}/10</p>
          </div>
      ...
      <div className="flex flex-col">
        ...
        {!loading && !result && (
          <div className="flex-1 flex flex-col items-center justify-center text-center px-6">
            <p className="font-bold text-base text-studio-ink">Fill out the brief on the left</p>
            <p className="text-sm text-studio-mutedSoft mt-2 leading-relaxed">
              After you have completed the brief, click <span className="font-bold text-studio-ink">Generate</span> to see three options to review <span className="font-bold text-studio-ink">here</span>.
            </p>
          </div>
        )}
        ...
        {loading && !result && (
          <div className="flex-1 flex flex-col gap-3">
            <p className="text-sm">
              <LoadingWords words={COMPOSE_LOADING_WORDS} className="italic text-studio-mutedSoft" />
            </p>
        ...
        {/* Result state — single variant (dot pager lives below the card, in
            the column wrapper) */}
        {result && enrichedVariations.length > 0 && activeVariant && (
          <div className="flex-1 flex flex-col min-h-0">
            <VariantCard
              ...
              onSendToRefine={() => onSendToRefine(activeVariant.copy, activeVariant.scores)}
              onCopy={() => handleCopy(activeVariant, safeIndex)}
              copied={copiedIndex === safeIndex}
              onRegenerate={() => handleRegenerateOne(activeVariant.differentiator, activeVariant.copy)}
              regenerating={regeneratingDiff?.toLowerCase() === activeVariant.differentiator.toLowerCase()}
            />
          </div>
        )}
        ...
        {result && enrichedVariations.length === 0 && (
          <div className="flex-1 flex items-center justify-center text-center px-6">
            <p className="text-sm text-studio-muted">No variations parsed from the response. Check the agent log.</p>
          </div>
        )}
        </section>
...
type EnrichedVariation = Variation & { scores: LensScores; word_count: number; fit: number }

function VariantCard({ index, variation, mandatories, onSendToRefine, onCopy, copied, onRegenerate, regenerating }: {
  index: number
  ...
  const headline = stripMarkdownEmphasis(rawHeadline)
  const body = stripMarkdownEmphasis(rawBody)
  ...
  return (
    <article className="bg-studio-page rounded-xl border border-studio-border p-6 flex flex-col flex-1 min-h-0">
      ...
      {/* Footer actions */}
      <footer className="flex items-center justify-between gap-2 pt-3 border-t border-studio-border text-xs text-studio-muted">
        <ActionBtn
          icon={regenerating ? <Loader2 className="h-3 w-3 animate-spin" /> : <RotateCcw className="h-3 w-3" />}
          label={regenerating ? 'Regenerating…' : 'Regenerate'}
          onClick={onRegenerate}
          disabled={regenerating}
        />
        <ActionBtn icon={<ArrowUpRight className="h-3 w-3" />} label="Send to Refine" onClick={onSendToRefine} />
        <ActionBtn icon={copied ? <Check className="h-3 w-3" /> : <CopyIcon className="h-3 w-3" />} label={copied ? 'Copied' : 'Copy'} onClick={onCopy} />
      </footer>
    </article>
  )
}
